...
export function SessionDetail() {
  ...
  if (!data) return <div className="p-6 text-text-mute">{t("app.loading")}</div>;
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md p-6">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 rounded-2xl bg-surface p-4 text-center text-sm">
        <div>
          <div className="text-xs text-text-mute">{t("workout.total_sets")}</div>
          <div className="ltr-numbers text-xl font-bold">{data.sets.length}</div>
        </div>
        <div>
          <div className="text-xs text-text-mute">{t("workout.total_volume")}</div>
          <div className="ltr-numbers text-xl font-bold">{Math.round(totalVolume)} kg</div>
        </div>
        {data.session.durationSeconds != null && (
          <div className="col-span-2">
            <div className="text-xs text-text-mute">{t("workout.duration")}</div>
      ...
      <ul className="mt-4 flex flex-col gap-2">
        {data.sets.map((s) => (
          <li key={s.id} className="flex items-center justify-between rounded-xl bg-surface px-4 py-3 text-sm">
