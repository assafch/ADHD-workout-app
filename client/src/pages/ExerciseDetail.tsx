...
export function ExerciseDetail() {
  ...
  if (!state) return <div className="p-6 text-stone-400">{t("app.loading")}</div>;
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md p-6">
      ...
      {data.length === 0 ? (
        <p className="mt-6 text-stone-400">{t("exercise.no_data")}</p>
      ) : (
        <>
          <div className="mt-4 h-64 w-full">
            <p className="mb-2 text-sm text-stone-300">{t("exercise.one_rm_trend")}</p>
          ...
          <ul className="mt-4 flex flex-col gap-2">
            {state.history.slice().reverse().map((h, idx) => (
              <li key={idx} className="flex items-center justify-between rounded-xl bg-stone-900 px-4 py-3 text-sm">
                <span>{h.date}</span>
